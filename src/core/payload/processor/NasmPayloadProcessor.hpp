...
namespace program::core::payload::nasm
{
    class CNasmPayloadProcessor : public IPayloadProcessor
    {
        ...
        const std::shared_ptr<IModificationCommand> processImports(const file_id& fileId, const IPayload* payload) const;
        const CDependencyGraph buildDependencyGraph(const file_id& fileId, const IPayload* payload) const;
        const std::shared_ptr<IModificationCommand> processData(const data_id& dataId, const file_id& fileId, const IPayload* payload) const;
        const std::shared_ptr<IModificationCommand> processProcedure(const procedure_id& procedureId, const file_id& fileId, const IPayload* payload) const;
        const std::shared_ptr<IModificationCommand> processPayloadChunks(const std::vector<std::string>& dependencies, const file_id& fileId, const IPayload* payload) const;
    public:
