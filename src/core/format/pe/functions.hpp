...
namespace program::core::format::pe {
    ...
    program::shared::types::uint_16 numberOfSections(const CPeFormat& peFormat);
    program::shared::types::binary_offset sectionsStartOffset(const CPeFormat& peFormat);
    pe_section_vec readSectionList(const CPeFormat& peFormat);
    program::shared::types::binary_offset rvaToOffset(const CPeFormat& peFormat, const program::shared::types::binary_offset rva);
    ...
    program::shared::types::uint_32 convertSectionPermissionsToCharacteristics(const program::core::shared::CSectionPermissions& permissions);
    program::core::shared::CSectionPermissions convertCharacteristicsToSectionPermissions(const program::shared::types::uint_32 characteristics);
    IMAGE_SECTION_HEADER createNextSectionHeader(
        const program::shared::types::uint_32 fileAlignment,
        const program::shared::types::uint_32 sectionAlignment,
