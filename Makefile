...
SHARED_OBJ_DIR=$(BUILD_DIR)/shared_obj
...
SHARED_OBJ=$(SHARED:%.cpp=$(SHARED_OBJ_DIR)/%.o)
...
clean:
	rm -rf $(RELEASE_DIR) $(DEBUG_DIR) $(RELEASE_DLL_DIR) $(DEBUG_DLL_DIR) $(SHARED_OBJ_DIR)

# --- SHARED part

$(SHARED_OBJ_DIR)/%.o: %.cpp
	@echo "[SHARED] Compiling: $< into $@"
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -O3 -c $< -o $@
...
$(RELEASE_DIR)/$(PE_NAME): $(RELEASE_OBJ) $(SHARED_OBJ)
	@echo "[RELEASE][LOADER] Linking: $^ into $@"
	@mkdir -p $(RELEASE_DIR)
	$(CC) $(LDFLAGS) -o $(RELEASE_DIR)/$(PE_NAME) $^

$(DEBUG_DIR)/$(PE_NAME)$(DEBUG_NAME_SUFFIX): $(DEBUG_OBJ) $(SHARED_OBJ)
...
$(RELEASE_DLL_DIR)/$(DLL_NAME).dll: $(DLL_RELEASE_OBJ) $(SHARED_OBJ)
	@echo "[RELEASE][DLL] Linking: $^ into $@"
	@mkdir -p $(RELEASE_DLL_DIR)
	$(CC) $(LDFLAGS) -shared -o $(RELEASE_DLL_DIR)/$(DLL_NAME).dll $^

$(DEBUG_DLL_DIR)/$(DLL_NAME)$(DEBUG_NAME_SUFFIX).dll: $(DLL_DEBUG_OBJ) $(SHARED_OBJ)
